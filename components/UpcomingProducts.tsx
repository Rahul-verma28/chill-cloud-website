                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 lg:gap-12">
                    {UPCOMING_PRODUCTS.map((product, idx) => {
                        return (
                            ...
                            >
                                {/* Card Container */}
                                <div className="relative h-full bg-[#111] p-4 rounded-3xl border border-white/5 overflow-hidden transition-all duration-700 hover:border-[#d4af37]/40 hover:bg-[#151515] hover:-translate-y-2 group-hover:shadow-[0_20px_40px_rgba(0,0,0,0.4)]">
                                    {/* Glass shine overlay */}
                                    <div className="absolute inset-0 bg-gradient-to-br from-white/5 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-700" />

                                    {/* Image Container (Circular) with Glow Animation */}
                                    <div className="relative w-56 h-56 mx-auto mb-5 rounded-full overflow-hidden border-[#d4af37]/50 backdrop-blur-xs transition-all duration-700 group-hover:scale-105">
                                        <motion.div
                                        ...
                                        >
                                            <Image
                                                ...
                                                alt={product.title}
                                                fill
                                                className="object-cover transition-all duration-1000 group-hover:scale-110 group-hover:opacity-100"
                                            />
                                        </motion.div>

                                        {/* Coming Soon Overlay */}
                                        <div className="absolute inset-0 flex items-center justify-center bg-black/40 group-hover:bg-black/10 backdrop-blur-[0.2px]" style={{ fontFamily: 'Playfair Display' }}>
                                            <div className="text-center transform transition-transform duration-700 group-hover:scale-110">
                                                <p className="text-3xl font-semibold tracking-[0.1em] text-white group-hover:text-[#d4af37]/80 drop-shadow-lg">Coming SOON</p>
                                            </div>
                                        </div>
                                    </div>
